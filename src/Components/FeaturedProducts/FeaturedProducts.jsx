  ...
  return (
    <section className="py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 📌 Category Highlights - 8 Latest Posts */}
        <div className="mb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {(() => {
              // Get latest 8 posts across all categories
              const categoryHighlights = products
                .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
                .slice(0, 8);

              return categoryHighlights.map((product) => {
                return (
                  <Card
                    key={product._id}
                  ...
                  >
                    ...
                    <CardHeader className="p-4 space-y-2 bg-gradient-to-b from-white to-gray-50">
                      {product.category && (
                        <div className="text-xs text-primary font-semibold uppercase tracking-wide">
                          {product.category}
                        </div>
                      )}
                      <CardTitle className="text-base font-bold line-clamp-2 text-gray-900 group-hover:text-primary transition-colors duration-300">
                        {product.title}
                      </CardTitle>
